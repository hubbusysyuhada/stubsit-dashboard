...
export default function EditEndpointDrawer(payload: EditEndpointDrawerPropsType) {
  ...
  const updateEndpoint = useFetch<string>(`${process.env.NEXT_PUBLIC_API_URL}/master/${group}/${payload.endpoint.slug}`, {
    method: 'PUT',
    headers: {
    ...
    <Drawer anchor='right' open={openDrawer} variant='soft' onClose={(_, reason) => closeDrawer(reason)}>
      <div className="p-8">
        ...
        <div className="mt-2">
          ...
          <Select value={selectedGroup} className="mt-2" onChange={(_, val) => setSelectedGroup(val || '')} >
            { renderOptions() }
          </Select>
        </div>
