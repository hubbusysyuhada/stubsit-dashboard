...
export default function Menu(payload: GroupDetailType) {
  ...
  const renderCalls = (e: BaseEndpointType) => {
    return e.calls.map((c) => {
      ...
      return (
        <div className={`flex ml-9 mt-2 cursor-pointer px-3 py-2 rounded-lg nav ${isCallActive ? 'active' : ''}`} onClick={() => router.push(`/${payload.slug}/${e.slug}/${c.slug}`)} key={`${payload.slug}-${e.slug}-${c.slug}`}>
    ...
    return payload.endpoints.map((e, index) => {
      ...
      return (
        <div key={`${payload.slug}-${e.slug}`}>
          <div className={`flex ml-5 items-center mt-2 cursor-pointer px-3 py-2 rounded-lg nav ${isEndpointActive ? 'active' : ''}`} onClick={() => router.push(`/${payload.slug}/${e.slug}`)}>
            <APIIcon className="text-sm" />
            <p className="ml-2 text-sm truncate">{e.name}</p>
          </div>
          <div className={`content calls ${isEndpointOpen ? 'open' : ''}`}>
            {renderCalls(e)}
          </div>
        </div>
      )
  ...
  return <div className="nav-dropdown">
    <div className="head">
      <div className={`flex items-center mt-3 cursor-pointer px-3 py-2 rounded-lg nav ${isGroupActive ? 'active' : ''}`} onClick={() => router.push(`/${payload.slug}`)}>
        <FolderIcon className="text-sm" />
        <p className="ml-2 text-sm truncate">{payload.name}</p>
      </div>
    </div>
    <div className={`content endpoints ${isGroupOpen ? 'open' : ''}`}>
      {renderEndpoints()}
    </div>
  </div>
}