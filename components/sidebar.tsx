...
export default function SidebarLayout() {
  ...
  const toLinkedin = () =>
    window.open('https://www.linkedin.com/in/hubbusysyuhada/', '_blank')?.focus();
  ...
  const renderMenu = () => {
    if (!groups.length && !isGroupsLoaded)
  ...
  return (
    <>
      <div
        id="sidebar"
        className="flex flex-col justify-between pb-9 relative !w-[360px] !min-w-[360px] px-4"
      >
        <div>
          <div
            className="relative w-[150px] h-[80px] icon hover:cursor-pointer"
            onClick={() => router.push('/')}
          >
            <Logo />
          </div>
          <div className="searchbar">
            <Input
              ...
              endDecorator={
                keyword ? (
                  <Cancel onClick={() => setKeyword('')} className="hover:cursor-pointer" />
                ) : null
              }
              className="my-4"
              value={keyword}
          ...
          <div className="menu max-h-[calc(100vh-280px)] overflow-y-auto">{renderMenu()}</div>
        </div>
      ...
      <CreateEndpointDrawer
        open={openCreatEndpointDrawer}
        onClose={() => setOpenCreateEndpointDrawer(false)}
      />
      <CreateGroupDrawer
        open={openCreatGroupDrawer}
        onClose={() => setOpenCreateGroupDrawer(false)}
      />
    </>
  );
}
