...
export default function EditCallDrawer(payload: EditCallDrawerPropsType) {
  ...
  const updateCall = useFetch<string>(
    `${process.env.NEXT_PUBLIC_API_URL}/master/${group}/${payload.call.endpoint.slug}/${payload.call.slug}`,
    {
      method: 'PUT',
      headers: {
    ...
    return (
      <div className="mt-3">
        ...
        <Editor
          ...
          onValidate={(markers) =>
            setIsResponseValidJSON(!markers.some((marker) => marker.severity === 8))
          }
          options={{
      ...
      >
        <div className="p-8">
          ...
          <div className="mt-3 flex">
            ...
            <Switch
              color={!is_error ? 'success' : 'danger'}
              onChange={() => setIsError(!is_error)}
              variant="soft"
            />
            <div
              className={`${is_error ? 'bg-[var(--method-delete)]' : 'bg-[var(--method-get)]'} text-[var(--background)] text-[10px] rounded-lg w-fit px-2 py-1 ml-3`}
            >
